<?php
    ...
    $db= mysqli_connect('localhost','jack','a1s2d3f4!','jack'); //DB주소, DB접속아이디, DB접속비번, DB명
    mysqli_query($db, 'set names utf8');  // 한글 깨짐 방지

    //원하는 쿼리문 수행(SQL언어 - web_board 테이블의 모든 데이터들 번호기준 내림차순으로..[최신글 순으로])
    $sql="SELECT * FROM web_board ORDER BY no DESC";
    $result=mysqli_query($db, $sql);

    //요청한 결과표($result)로 부터 게시글들을 한 줄씩 가져와서 $board_list 라는 이름의 배열에 추가하기
    $board_list=[];  // 빈 배열 준비
    // DB에서 데이터를 가져오는 것은 한 번에 한 줄씩 가져와야 함.. 그래서 반복문 필요.
...
<!-- php는 결국 사용자가 보는 페이지를 echo하기에, html태그문을 그냥 쓰면 자동 echo -->
...
<body>
    ...
    <div class="board_wrap">
        ...
        <!-- 2. 게시들 테이블이 그려질 메인 영역 ( 테이블, 페이지네이션, 등록버튼) -->
